...
static float numeric_gelu_grad(const RMatrix *input, const RMatrix *upstream, size_t idx)
{
...
static float numeric_swish_grad(const RMatrix *input, const RMatrix *upstream, size_t idx)
{
	const float delta = 1e-4f;
...
static float numeric_softmax_grad(const RMatrix *input, const RMatrix *upstream, size_t idx)
{
	const float delta = 1e-4f;
...
static void test_relu_grad_basic_values(void)
{
	...
	assert_matrix_close(grad, expected_vals, 4, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "relu_grad_basic");
...
static void test_relu_grad_nan_input_zeroed(void)
{
	...
	assert_matrix_close(grad, expected_vals, 2, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "relu_grad_nan");
...
static void test_leaky_relu_grad_basic_values(void)
{
	...
	assert_matrix_close(grad, expected_vals, 3, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "leaky_relu_grad_basic");
...
static void test_leaky_relu_grad_alpha_zero_is_relu(void)
{
	...
	assert_matrix_close(grad, expected_vals, 2, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "leaky_relu_grad_alpha_zero");
...
static void test_gelu_grad_zero_input(void)
{
	...
	assert_matrix_close(grad, expected_vals, 1, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, "gelu_grad_zero");
...
static void test_softmax_grad_numeric(void)
{
	...
	for (size_t row = 0; row < grad->rows; row++)
	{
		...
		{
			sum += grad->data[RMatrixIDX(row, col, grad->cols)];
		}
		char msg[96];
		snprintf(msg, sizeof(msg), "softmax_grad_row_sum row=%zu", row);
		assert_float_close(sum, 0.0f, DEFAULT_ABS_TOL, DEFAULT_REL_TOL, msg);
	}
...
static void test_zero_sized_matrices_no_crash(void)
{
	...
	r_activation_leaky_relu_grad(input_rows0, upstream_rows0, 0.1f, grad_rows0);
	...
	assert_true(grad_rows0->rows == 0 && grad_rows0->cols == 3, "zero_rows preserves shape");
	...
	r_activation_leaky_relu_grad(input_cols0, upstream_cols0, 0.1f, grad_cols0);
	...
	assert_true(grad_cols0->rows == 2 && grad_cols0->cols == 0, "zero_cols preserves shape");
